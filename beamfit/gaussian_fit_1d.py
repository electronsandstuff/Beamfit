...
class GaussianProfile1D(AnalysisMethod):
    ...
    def fit(self, image):
        ...
        s2 = np.sqrt(2)
        x_amplitude_scale = 2 / (
                    special.erf((image.shape[1] - yfit[0]) / yfit[1] / s2) - special.erf(-yfit[0] / yfit[1] / s2))
        x_amplitude_scale /= yfit[1] * np.sqrt(2 * np.pi)
        y_amplitude_scale = 2 / (
                    special.erf((image.shape[0] - xfit[0]) / xfit[1] / s2) - special.erf(-xfit[0] / xfit[1] / s2))
...
def fit_gaussian_1d(image):
    """This function is for backwards compatibility with old code"""
    o = GaussianProfile1D()
    return o.fit(image)

...
class TestBeamfit(unittest.TestCase):
    ...
    def test_fit_supergaussian(self):
        ...
        res = beamfit.SuperGaussian(batch_size=0.03).fit(test_image)
        np.testing.assert_allclose(res.h, valid_h, rtol=0.2)
        res = beamfit.SuperGaussian(batch_size=0.03).fit(test_image, np.ones_like(test_image))
        np.testing.assert_allclose(res.h, valid_h, rtol=0.2)
    ...
    def test_grads_numerical(self):
        ...
        for p, h in zip(self.ps, hs):
            try:
                ...
                #print(err)
                self.assertTrue((err[np.isfinite(err)] < 1e-6).all())  # If it fails here, need to make h smaller
